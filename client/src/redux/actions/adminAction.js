...
export const allAccUsers =
	(currentPage = 1) =>
	async (dispatch) => {
		try {
			dispatch({ type: 'ALL_USERS_REQUEST' });
			let URL = `/api/admin/getAllAccount?page=${currentPage}`;
	...
	async (dispatch) => {
		try {
			...
			const { data } = await axios.get(
				`/api/admin/getAccountStaff?page=${currentPage}`
			);

			dispatch({ type: 'ACC_STAFF_SUCCESS', payload: data });
		} catch (error) {
			dispatch({
				type: 'ACC_STAFF_FAIL',
				payload: error.response.data.message,
